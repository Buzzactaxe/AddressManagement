...
public class Ui {
    ...
    public void displayAllContacts(Scanner scanner) {
        ...
        } else System.out.println("There are no contacts available");

        showExitUi(scanner);
    }

    public void showContactAgeUi(Scanner scanner) {
        var contactAgeList = contactService.getContactList();
        if (!contactAgeList.isEmpty()) {
    ...
    public void findContactById() throws IOException {
        var contactList = contactService.getContactList();
        if (!contactList.isEmpty()) {
            Scanner scanner = new Scanner(System.in);
            System.out.println("Enter  ID of Contact you want to find\n" + "Please Enter integer numbers");
            var customerId = removeAlphabetChars(scanner.nextLine());
            while (customerId.isBlank()) {
                System.out.println("Incorrect data, num ID required to find contact.");
                customerId = removeAlphabetChars(scanner.nextLine());
            }
            if (contactService.findById(customerId).isPresent()) {
                System.out.println(contactService.findById(customerId).get());
            } else {
                System.out.println("The contact requested is not available");
            }
